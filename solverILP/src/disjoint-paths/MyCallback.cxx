...
MyCallback::MyCallback(disjointPaths::ilp::Gurobi& solver, Data<>& data,bool outputs) :
...
void MyCallback::separateAndAddLazyConstraints(){
	...
	if(useOutputs){
...

		{
			...
			if(data_.getTask()=='T'){
				optFileName=data_.parameters.getOutputFileName() + "-tr-optimization-log.txt";
			}
			else{
				optFileName=data_.parameters.getOutputFileName() + "-optimization-log.txt";
			}
	...
	if (numberOfAddedConstraints == 0)
	{
		...
		if(data_.parameters.isDebugOutputFiles()&&useOutputs){

			std::ofstream file;
			char task=data_.getTask();
			std::string suffix="";
			if(task=='I'){
				suffix="int";
			}
			...
			file.open(data_.parameters.getOutputFileName() +"-"+suffix+ "-all-paths-FEASIBLE-" + std::to_string(numberOfFeasibleSolutions_) + ".txt");
...
void MyCallback::simplePathConstraint(size_t w,size_t& counter,std::list<size_t> & path, std::vector<bool> &isOnPath,andres::graph::Digraph<> const & graph,andres::graph::Digraph<> const & liftedGraph){
	...
	size_t MyCallback::preparePathConstraint(size_t vCurrent,size_t vertex,size_t& counter,std::unordered_map<size_t,size_t>& pred,std::unordered_map<size_t,bool>& predIsLifted, andres::graph::Digraph<> const & graph,andres::graph::Digraph<> const & liftedGraph,size_t w){
	...
	void MyCallback::preparePathConstrReversed(size_t vCurrent,size_t vertex,size_t& counter,std::unordered_map<size_t,size_t>& pred,std::unordered_map<size_t,bool>& predIsLifted, andres::graph::Digraph<> const & graph,andres::graph::Digraph<> const & liftedGraph){
	...
	void MyCallback::prepareCutReversed(size_t vCurrent,size_t vertex,size_t w,size_t& counter,std::unordered_map<size_t,size_t>& pred,std::unordered_map<size_t,bool>& predIsLifted, andres::graph::Digraph<> const & graph,andres::graph::Digraph<> const & liftedGraph){
		//std::set<size_t> usedVertices;
		std::vector<bool> used(graph.numberOfVertices(),0);
	...
	size_t MyCallback::addMixedConstraints(){
			size_t constraintsCounter=0;
			size_t t=data_.getTerminalNode();
			size_t s=data_.getSourceNode();
			andres::graph::Digraph<> const& graph=data_.getGraph();  //TODO check if the const & is correct
			...
			for (int i = 0; i < graph.numberOfEdgesToVertex(t); ++i) {
				...
				if(this->label(eVarIndex) > 0.5){
					std::vector<bool> isOnPath(graph.numberOfVertices(),0);
					std::unordered_map<size_t,size_t> pred;
					std::unordered_map<size_t,bool> predIsLifted;
					std::unordered_map<size_t,size_t> predNeg;
					size_t vertex=graph.vertexToVertex(t,i);
					std::list<size_t> path;
						...
						for (int j = 0; j < liftedGraph.numberOfEdgesFromVertex(vertex); ++j) {
							...
							if(w==oldVertex){
								...
								if(isActive){
									...
									if(isOnPath[w]&&!isActive){
									//	std::cout<<w<< " is on path and not active "<<std::endl;

										size_t counter=0;
										size_t vCurrent=w;

										size_t tempCounter=preparePathConstraint(vCurrent,vertex,counter,pred,predIsLifted,graph,liftedGraph,vCurrent);

										variables_[tempCounter]=leVarIndex; //replace the last vertex variable with lifted edge variable
										coefficients_[tempCounter]=1;


										this->addLazyConstraint(variables_.begin(),	variables_.begin() + counter ,
												coefficients_.begin(),0,std::numeric_limits<double>::infinity());
										constraintsCounter++;
										//std::cout<<"positive lifted added "<<std::endl;

									}
									//else if(!isOnPath[w]&&isActive&&uncoveredFalsePositive[vertex].count(w)>0){
								    else if(!isOnPath[w]&&isActive){
										bool cutApproach=false;
										//
										if(predNeg.count(w)>0){
											...
											preparePathConstraint(vCurrent,vertex,counter,pred,predIsLifted,graph,liftedGraph,w);
											//std::cout<<std::endl;
											...
											//else if(uncoveredFalsePositive[vertex].count(w)>0){
											...
											size_t counter=0;
											size_t vLast=*itv;   //w unreachable from vLast

											size_t vCurrent=*itv;
											size_t edgeIndex=0;
											size_t edgeVarIndex=0;

											//if(!predIsLifted[*itv]||data_.isReachable(w,*itv)){ //Comment here for using cut always
											itv--;
											vCurrent=*itv;  //w reachable from vCurrent


											for (int i = 0; i < graph.numberOfEdgesFromVertex(vCurrent); ++i) {
												size_t endVertex=graph.vertexFromVertex(vCurrent,i);
												//if(endVertex!=w&&!data_.isReachable(endVertex,w)){
												if(endVertex==w||data_.isReachable(endVertex,w)){
													edgeIndex=graph.edgeFromVertex(vCurrent,i);
													edgeVarIndex=data_.getEdgeVarIndex(edgeIndex);
													variables_[counter]=edgeVarIndex;
													//coefficients_[counter]=-1;
													coefficients_[counter]=1;
											...
											//}  //Comment here for using cut always
										...
										//std::cout<<" negative lifted added"<<std::endl;
